# AGENTS.md - AI Agent Development Guide for tap-qomon
...
This tap follows the Singer specification and uses the Hotglue Singer SDK (hotglue_singer_sdk) to extract data from Qomon.

### Key Components

1. **Tap Class** (`tap_qomon/tap.py`): Main entry point, defines streams and configuration
2. **Client** (`tap_qomon/client.py`): HTTP client, stream base class, and (where applicable) how auth is attached to requests
3. **Streams** (`tap_qomon/streams.py`): Stream classes, schemas (`th.PropertiesList`), and—depending on stream type—`path`, `query`, or custom extraction hooks
4. **Custom fields** (`tap_qomon/custom_fields.py`): Discovery, schema extension, and record flattening for Qomon custom fields

### Custom fields are discovered, not hardcoded

Qomon spaces define their own custom fields, so the `contacts` schema is built at
discover time rather than declared statically:

1. `TapQomon.discover_streams()` calls `ContactsStream.fetch_custom_field_definitions()`,
   which reads `GET /v1/forms/type/custom_fields`.
2. `extend_properties_with_custom_fields()` appends one property per definition, keyed by
   the form's label. Labels that collide with a base contact field are skipped.
3. The definitions are cached on the tap as `custom_field_definitions_by_id`, and
   `ContactsStream.post_process()` uses them to unfurl each contact's `custom_fields` /
   `formdatas` answers onto top-level properties.

When changing the base contact schema, remember that a new base property silently takes
precedence over a same-named custom field.

### Contacts are read through the search API

Qomon has no contact listing endpoint. `ContactsStream` POSTs to `/search` with an
`advanced_search` body; `prepare_request_payload()` builds the query (including the
`UpdatedAt` incremental filter) and `get_next_page_token()` pages until a short page
arrives, since the API returns no total count.
...
1. Add a stream class in `tap_qomon/streams.py` (follow naming: PascalCase + `Stream`, Singer **`name`** in snake_case—consistent with existing streams).
...
5. Add the class to **`STREAM_TYPES`** in `tap.py` and to the **`from tap_qomon.streams import (...)`** block (Cookiecutter pre-fills both from **`stream_names`**).
...
The agent should align **`name`**, **`path`** (or **`query`** for GraphQL), **`schema`**, **`records_jsonpath`** / **`get_next_page_token()`** in **`tap_qomon/client.py`** when needed, and `discover_streams()` + imports in **`tap.py`** for new streams. See **Handling Pagination** and **Adding a New Stream** above.

#### Modifying Authentication

- Token read from the **`api_key`** config property (see `tap.py` / `README.md`), matching the setting name used by `target-qomon`
- Passed via **`BearerTokenAuthenticator`** in **`client.py`** (`Authorization: Bearer …`)

#### Handling Pagination

The scaffold pages over HTTP using **`get_next_page_token()`** on the stream class (see **`tap_qomon/client.py`** for the default implementation using JSONPath / headers). Return the value the SDK should pass on the next request (page number, cursor, URL, etc.); return **`None`** when there is no next page. **GraphQL** APIs often expose cursors inside the JSON body—parse those here and thread the token into the request builder your client uses.
...
```bash
tap-qomon --config config.json --discover
tap-qomon --config config.json --catalog catalog.json
```

### Keeping tap config, docs, and env in sync

Authoritative settings live in **`config_jsonschema`** on the tap class in `tap_qomon/tap.py` (Hotglue Singer SDK). Everything operators and Hotglue jobs read should match that schema.
...
1. Update `config_jsonschema` in `tap_qomon/tap.py`.
2. Update **`README.md`**: the configuration table, the example `config.json`, and any prose that names settings.
3. Update **`.env.example`** so variable names stay aligned with `tap-qomon --about` / `--config=ENV` (keys are derived from config property names).
...
TAP_QOMON_API_URL=https://api.example.com
TAP_QOMON_ACCESS_KEY=your_access_key_here
TAP_QOMON_BATCH_SIZE=100
```
...
- Prefer `tap-qomon --about` (or `--format=markdown`) as the generated reference for your tree.
...
Before filing, search existing issues to avoid duplicates. Include the SDK version (`tap-qomon --version` with your venv activated), Python version, and a minimal reproduction case when reporting bugs.
